...
A calendar component for selecting single dates, date ranges, or years. Omit `value` / `rangeValue` for local state, or provide the matching change callback to control selection. Controlled values follow external updates, including resets.
...
<div className="space-y-4">
  ...
  <div>
    ...
    <DatePicker value={checkOut} onChange={setCheckOut} minDate={checkIn ?? undefined} />
  </div>
</div>;
```
...
| `defaultValue`      | `Date \| null`                                  | —          | Initial uncontrolled date        |
| `defaultRangeValue` | `[Date \| null, Date \| null]`                  | —          | Initial uncontrolled range       |
...
- Arrow keys move between days; Home/End move to the week edges, Page Up/Down change months, and Enter/Space select.
- Each month has one day in the Tab order; disabled dates are skipped.
