---
title: Icon
description: 200+ icons with outline and solid variants
---

# Icon

Rayden UI includes 200+ icons with outline and solid variants in multiple sizes.

## Import

```tsx
import { Icon } from "@raydenui/ui";
```

## Loading and server rendering

For immediate rendering, including server-rendered markup, pass static icon data. Only the imported icon is retained by a tree-shaking bundler:

```tsx
import { Icon } from "@raydenui/ui";
import { homeIcon } from "@raydenui/ui/icons";

<Icon icon={homeIcon} />;
```

The convenient `name` API loads the shared icon catalog after mounting in the browser. Until it arrives, the SVG reserves its final size. The catalog is a separate chunk in bundlers that support code splitting; it is not needed for a plain Button. Use static icon data for icons that must appear in the initial HTML.

## Usage

### Basic

<Preview>
  <Icon name="home" />
  <Icon name="user" />
  <Icon name="settings" />
  <Icon name="bell" />
  <Icon name="mail" />
</Preview>

```tsx
<Icon name="home" />
```

### Variants

<Preview>
  <Icon name="heart" variant="outline" />
  <Icon name="heart" variant="solid" />
  <Icon name="star" variant="outline" />
  <Icon name="star" variant="solid" />
</Preview>

```tsx
// Outline (default)
<Icon name="home" variant="outline" />

// Solid
<Icon name="home" variant="solid" />
```

### Sizes

<Preview>
  <Icon name="home" size="xs" />
  <Icon name="home" size="sm" />
  <Icon name="home" size="md" />
  <Icon name="home" size="lg" />
  <Icon name="home" size="xl" />
</Preview>

```tsx
<Icon name="home" size="xs" /> {/* 12px */}
<Icon name="home" size="sm" /> {/* 16px */}
<Icon name="home" size="md" /> {/* 20px */}
<Icon name="home" size="lg" /> {/* 24px */}
<Icon name="home" size="xl" /> {/* 32px */}
```

### Custom Color

<Preview>
  <Icon name="heart" className="text-primary-500" />
  <Icon name="check-circle" className="text-success-400" />
  <Icon name="multiply-circle" className="text-error-400" />
  <Icon name="info-circle" className="text-warning-400" />
</Preview>

```tsx
<Icon name="heart" className="text-primary-500" />
<Icon name="check-circle" className="text-success-400" />
<Icon name="multiply-circle" className="text-error-400" />
```

## Available Icons

### Navigation

- `home`, `home-alt`, `x`, `chevron-left`, `chevron-right`, `chevron-up`, `chevron-down`
- `arrow-left`, `arrow-right`, `arrow-up`, `arrow-down`
- `link`, `link-detach`, `log-out`

### Actions

- `plus`, `minus`, `pencil`, `pencil-edit`, `bin`, `copy`, `download`, `upload`
- `search`, `filter`, `settings`, `settings-1`, `settings-2`

### Status

- `check`, `check-circle`, `x-circle`, `info-circle`
- `bell`, `star`, `heart`, `bookmark`

### Content

- `file`, `folder`, `image`, `calendar`, `clock`, `map-marker`, `globe`, `mail`

### Users

- `user`, `users`, `user-add`, `user-cross`, `user-circle`

## Props

| Prop        | Type                                   | Default     | Description                                              |
| ----------- | -------------------------------------- | ----------- | -------------------------------------------------------- |
| `name`      | `IconName`                             | —           | Icon name; use either `name` or `icon`                   |
| `icon`      | `IconRecord`                           | —           | Static icon data for immediate, tree-shakeable rendering |
| `variant`   | `"outline" \| "solid"`                 | `"outline"` | Icon variant                                             |
| `size`      | `"xs" \| "sm" \| "md" \| "lg" \| "xl"` | `"md"`      | Icon size                                                |
| `className` | `string`                               | —           | Additional CSS classes                                   |

## Accessibility

- Icons are decorative by default (`aria-hidden="true"`)
- For standalone icons with meaning, use `aria-hidden={false}`, `role="img"`, and an `aria-label`.
