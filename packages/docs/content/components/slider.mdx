...
<Preview vertical>
  <Slider label="Download Speed" metadata="50 MB/s" defaultValue={50} />
  <Slider label="Connection" metadata={["5G", "Low latency", "Stable"]} defaultValue={75} />
</Preview>
...
<Preview>
  <RangeSlider min={0} max={1000} step={50} label="Budget ($)" defaultValue={[200, 800]} />
</Preview>
...
- Tab focuses each thumb; arrows adjust by one step, Page Up/Down by ten steps, and Home/End move to its allowed bounds.
- Range endpoints are named separately and cannot cross.
