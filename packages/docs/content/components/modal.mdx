...
- Uses a native `<dialog>` opened with `showModal()`
- Title linked via `aria-labelledby`
- Description linked via `aria-describedby`
- Close button has `aria-label="Close modal"`
- The browser contains focus and makes the background inert while the dialog is open
- Closing returns focus to the trigger when it is still available
