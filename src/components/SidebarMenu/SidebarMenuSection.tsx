...
// ─── Component ────────────────────────────────────────────────────
export const SidebarMenuSection = forwardRef<
  HTMLDivElement,
...
>(({ title, showDivider = true, children, className, ...rest }, ref) => {
  ...
  return (
    <div ref={ref} className={cn("flex flex-col", className)} {...rest}>
      {/* Section title */}
      {title && !collapsed && (
        <span className={cn("mb-2 px-4 text-body-xs font-medium uppercase tracking-wider", ts.sectionTitle)}>
          {title}
        </span>
      )}
      ...
      {/* Divider */}
      {showDivider && (
        <div className={cn("mt-3 border-b", ts.divider)} />
      )}
    </div>
  );
});
